...
load("~/ErnstData/Framing/6_Model/models_toRun/HaDDM2t/results_HaDDM_Fit_ALL_allfree_newmod_thetaFree.RData")
rm(Data_noAtt)
...
for (s in subs){
  ...
  biasG = mean(chain1[,c( paste( c("biasG[",toString(countSub),"]"), collapse = ""))])
  ...
  biasL = mean(chain1[,c( paste( c("biasL[",toString(countSub),"]"), collapse = ""))])
...
print(pp)
#dev.off()
save(results,dataSim, Data,file='~/ErnstData/Framing/6_Model/models_toRun/HaDDM2t/simulations_HaDDM_allfree_newmod_allf.Rdata')
