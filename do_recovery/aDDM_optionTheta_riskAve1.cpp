...
struct ddm2w : public Worker { 
  ...
  ddm2w(const double drift, const double thres, const double nDT, const double thetaGaze_A, const double thetaGaze_B, const double bias, const double val_A_scaled, const double val_B_scaled, const double prob_A, const double prob_B, std::vector<double> fixL, std::vector<double> fixDur, const double sd_n, NumericVector vecOut , GEN gen)
    : drift(drift), thres(thres), nDT(nDT), thetaGaze_A(thetaGaze_A), thetaGaze_B(thetaGaze_B), bias(bias), val_A_scaled(val_A_scaled), val_B_scaled(val_B_scaled), prob_A(prob_A), prob_B(prob_B), fixL(fixL), fixDur(fixDur), sd_n(sd_n), gen(gen), vecOut(vecOut) {}
  ...
  void operator()(std::size_t begin, std::size_t end) {
    ...
    for (std::size_t i = begin; i < end; i++) {
      ...
      while (flag==0 && cont<lt) {
        ...
        X = X + ( drift*val_A_scaled*prob_A*vec_L[cont] - drift*val_B_scaled*prob_B*vec_R[cont] )*dt + noise;
...
// [[Rcpp::export]]
NumericVector ddm2_parallel(double drift, double thres, double nDT, double thetaGaze_A, double thetaGaze_B, double bias, double val_A_scaled, double val_B_scaled, double prob_A, double prob_B, std::vector<double> fixL, std::vector<double> fixDur, double sd_n, unsigned int N) {
  ...
  // create the worker
  ddm2w ddm2w(drift, thres, nDT, thetaGaze_A, thetaGaze_B, bias, val_A_scaled, val_B_scaled, prob_A, prob_B, fixL, fixDur, sd_n, vecOut, gen);
