# 2022/08/27 add val_scale_factor
# 2022/08/27 use typename to call Rcpp file
# 2022/08/27 remove bias
...
# ===================================================
val_scale_factor <-300

# folders
input_folder <- "result_fitting"
typename <- "optionTheta_noBias"
designSepc <- "2money" #for calling associate files
mydata <-  'krajbich_risk'
scaleSpec <- paste("scale", toString(val_scale_factor), sep="")
filename <- paste(typename, "_", mydata,"_" , scaleSpec , sep="")
...
sourceCpp(paste(recovery_folder, "/aDDM_", typename, "_", designSepc ,".cpp" ,sep=""))
# ==========================================================================
### Simulations ###
...
data_trial$val_A1_scaled <- data_trial$val_A1/val_scale_factor
data_trial$val_B1_scaled <- data_trial$val_B1/val_scale_factor
data_trial$val_A2_scaled <- data_trial$val_A2/val_scale_factor
data_trial$val_B2_scaled <- data_trial$val_B2/val_scale_factor



#only options and subject id from data_trial
sim_data<- data.frame(val_A1_scaled = data_trial$val_A1_scaled, val_B1_scaled = data_trial$val_B1_scaled, 
                      val_A2_scaled = data_trial$val_A2_scaled, val_B2_scaled = data_trial$val_B2_scaled, 
...
for (s in unique(sim_data$subj_idx)){  # s is subject index (from 1~N)
  ...
  #sim_data$bias[sim_data$subj_idx==s]  = mean(chain[,c( paste( c("bias[",toString(s),"]"), collapse = ""))]) 
  ...
  # get options
  val_A1_scaled = sim_data$val_A1_scaled[sim_data$subj_idx==s]
  val_B1_scaled = sim_data$val_B1_scaled[sim_data$subj_idx==s]
  val_A2_scaled = sim_data$val_A2_scaled[sim_data$subj_idx==s]
  val_B2_scaled = sim_data$val_B2_scaled[sim_data$subj_idx==s]
  ...
  for (i in 1:length(val_A1_scaled)) { # for each trial of this subject
    ...
    ######################## here using thres =2
    rt <- ddm2_parallel(drift[i], rho[i], 2, nDT[i], thetaGaze_A[i], thetaGaze_B[i],  
                        val_A1_scaled[i], val_B1_scaled[i], val_A2_scaled[i], val_B2_scaled[i], 
                        fixL, fixDur, noise[i], 1)
    sim_data$RT_PN[sim_data$subj_idx==s & sim_data$val_A1_scaled==val_A1_scaled[i] & sim_data$val_B1_scaled==val_B1_scaled[i] & sim_data$val_A2_scaled==val_A2_scaled[i] & sim_data$val_B2_scaled==val_B2_scaled[i]] = rt
    ...
    sim_data$fixDur_A_sum[sim_data$subj_idx==s & sim_data$val_A1_scaled==val_A1_scaled[i] & sim_data$val_B1_scaled==val_B1_scaled[i] & sim_data$val_A2_scaled==val_A2_scaled[i] & sim_data$val_B2_scaled==val_B2_scaled[i]] = sum(fixDur_real[fixL_real==1],na.rm=TRUE)
    sim_data$fixDur_sum[sim_data$subj_idx==s & sim_data$val_A1_scaled==val_A1_scaled[i] & sim_data$val_B1_scaled==val_B1_scaled[i] & sim_data$val_A2_scaled==val_A2_scaled[i] & sim_data$val_B2_scaled==val_B2_scaled[i]] = ToTfix_time
  }
}
