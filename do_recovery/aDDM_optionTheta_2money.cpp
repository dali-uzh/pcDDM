...
struct ddm2w : public Worker { 
  ...
  ddm2w(const double drift, const double rho, const double thres, const double nDT, const double thetaGaze_A, const double thetaGaze_B, const double bias, const double val_A1, const double val_B1, const double val_A2, const double val_B2, std::vector<double> fixL, std::vector<double> fixDur, const double sd_n, NumericVector vecOut , GEN gen)
    : drift(drift), rho(rho), thres(thres), nDT(nDT), thetaGaze_A(thetaGaze_A), thetaGaze_B(thetaGaze_B), bias(bias), val_A1(val_A1), val_B1(val_B1), val_A2(val_A2), val_B2(val_B2), fixL(fixL), fixDur(fixDur), sd_n(sd_n), gen(gen), vecOut(vecOut) {}
  ...
  void operator()(std::size_t begin, std::size_t end) {
    ...
    for (std::size_t i = begin; i < end; i++) {
      ...
      while (flag==0 && cont<lt) {
        ...
        X = X + ( drift*( (pow(val_A1,rho)+pow(val_A2,rho))*0.5 )*vec_L[cont] - drift*( (pow(val_B1,rho)+pow(val_B2,rho))*0.5 )*vec_R[cont] )*dt + noise;
...
// [[Rcpp::export]]
NumericVector ddm2_parallel(double drift, double rho, double thres, double nDT, double thetaGaze_A, double thetaGaze_B, double bias, double val_A1, double val_B1, double val_A2, double val_B2, std::vector<double> fixL, std::vector<double> fixDur, double sd_n, unsigned int N) {
  ...
  // create the worker
  ddm2w ddm2w(drift, rho, thres, nDT, thetaGaze_A, thetaGaze_B, bias, val_A1, val_B1, val_A2, val_B2, fixL, fixDur, sd_n, vecOut, gen);
