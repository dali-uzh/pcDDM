...
struct ddm2w : public Worker { 
  ...
  ddm2w(const double drift, const double rho, const double thres, const double nDT, const double thetaGaze_A, const double thetaGaze_B, const double bias, const double val_A_scaled, const double val_B_scaled, const double prob_A, const double prob_B, std::vector<double> fixL, std::vector<double> fixDur, const double sd_n, NumericVector vecOut , GEN gen)
    : drift(drift), rho(rho), thres(thres), nDT(nDT), thetaGaze_A(thetaGaze_A), thetaGaze_B(thetaGaze_B), bias(bias), val_A_scaled(val_A_scaled), val_B_scaled(val_B_scaled), prob_A(prob_A), prob_B(prob_B), fixL(fixL), fixDur(fixDur), sd_n(sd_n), gen(gen), vecOut(vecOut) {}
...
// [[Rcpp::export]]
NumericVector ddm2_parallel(double drift, double rho, double thres, double nDT, double thetaGaze_A, double thetaGaze_B, double bias, double val_A_scaled, double val_B_scaled, double prob_A, double prob_B, std::vector<double> fixL, std::vector<double> fixDur, double sd_n, unsigned int N) {
  ...
  // create the worker
  ddm2w ddm2w(drift, rho, thres, nDT, thetaGaze_A, thetaGaze_B, bias, val_A_scaled, val_B_scaled, prob_A, prob_B, fixL, fixDur, sd_n, vecOut, gen);
